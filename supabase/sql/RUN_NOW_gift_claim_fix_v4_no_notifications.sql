-- =====================================================================
-- 🎁 GIFT SYSTEM V4 — CHẠY TRÊN SUPABASE #1 (KHÔNG CÓ BẢNG notifications)
--
-- Khác biệt so với V3:
--   • BỎ HOÀN TOÀN mọi DROP TRIGGER ... ON public.notifications.
--   • KHÔNG SELECT / INSERT / UPDATE / DELETE public.notifications.
--   • KHÔNG giả định bảng notifications tồn tại trên SB1.
--   • KHÔNG tham chiếu cột post_gifts.notif_id (schema hiện tại không còn).
--   • Mọi dọn dẹp function/trigger cũ đều dùng IF EXISTS.
--
-- SB1 chỉ xử lý: ví (profiles.gem_balance), post_gifts, message_gifts,
-- gem_transactions. Notification hoàn toàn thuộc SUPABASE #3 (client tạo).
--
-- Chạy nguyên khối trong SQL Editor của SUPABASE #1.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 0) Dọn function guard cũ của luồng notification (nếu còn sót lại).
--    Không đụng tới bảng notifications, không DROP TRIGGER trên bảng đó.
-- ---------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.guard_pending_gift_notification() CASCADE;

-- ---------------------------------------------------------------------
-- 1) GỬI QUÀ BÀI VIẾT — send_post_gift_v2
-- ---------------------------------------------------------------------
...
--    notif_id KHÔNG được tham chiếu; trả về null để client SB3 tự xử lý.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object('ok', true, 'code', 'CLAIMED',
    'gift_id', p_gift_id, 'notif_id', NULL,
...
-- ---------------------------------------------------------------------
-- 3) NHẬN QUÀ TRONG CHAT — claim_message_gift
-- ---------------------------------------------------------------------
