...
--
-- File này gộp 2 bước thành 1:
--   1) Sửa trigger guard để honor flag `app.allow_gem_change = '1'`
--      → Frontend update gem_balance vẫn bị CHẶN
--      → RPC SECURITY DEFINER bật flag thì ĐƯỢC PHÉP
--   2) Cài 2 RPC V3 mà frontend đang gọi:
--        - public.gift_gem_to_post_v3(uuid, bigint)
--        - public.claim_gift_gem_v3(uuid)
--
-- KHÔNG check Admin. Ai cũng tặng/nhận được. Người nhận PHẢI bấm Nhận.
-- =====================================================================
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN v_allow := NULL; END;
  IF COALESCE(v_allow, '') = '1' THEN RETURN NEW; END IF;
...
-- ---------- 2) RPC V3 ----------
-- Nội dung đầy đủ của 2 RPC nằm ở:
--   supabase/sql/20260606_v3_two_phase_gift.sql
-- Hãy chạy NGUYÊN FILE đó NGAY SAU file này để cài RPC.
--
-- Hoặc copy nguyên block từ file đó dán xuống đây nếu muốn 1-file-all-in-one.-- =====================================================================
