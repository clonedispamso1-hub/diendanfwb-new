...
-- =============================================================================
-- FINAL 2026-08-23 — my_cash_flow() đúng schema thật + siết bảo mật admin
...
--     → dùng đúng sender_id / receiver_id / kind (không có cột cũ nào khác).
...
-- -----------------------------------------------------------------------------
-- 4) my_cash_flow(p_days) — DÒNG TIỀN theo CỘT THẬT + tên/UID đối tác
--    Gồm: rút tiền (kèm status), chuyển xu đi, nhận xu về — mặc định 3 ngày.
--    Loại trừ: quà tặng bài viết / tip / bút toán admin trong gem_transactions.
--    Cột đối tác: counterparty_id / counterparty_name / counterparty_public_id.
-- -----------------------------------------------------------------------------
...
begin
  ...
  with rows_all as (
    ...
    select case when t.sender_id = v_uid then 'transfer_out' else 'transfer_in' end::text,
           ...
           t.note,
           t.created_at,
           case when t.sender_id = v_uid then t.receiver_id else t.sender_id end
    ...
    -- 4.3 Sổ cái ví (gem_transactions) — bỏ quà tặng bài viết / tip / admin
    select case when g.sender_id = v_uid then 'transfer_out' else 'transfer_in' end::text,
           ...
           null::text,
           g.created_at,
           case when g.sender_id = v_uid then g.receiver_id else g.sender_id end
  ...
  select r.kind, r.id, r.code, r.amount, r.fee, r.net_amount,
         ...
         r.cp_id,
         nullif(btrim(coalesce(p.full_name, '')), '') as counterparty_name,
         nullif(btrim(coalesce(p.public_id::text, '')), '')       as counterparty_public_id
    from rows_all r
    left join public.profiles p on p.id = r.cp_id
...
--   select * from public.my_cash_flow();                     -- chỉ thấy giao dịch của mình
--   select kind, counterparty_name, counterparty_public_id
--     from public.my_cash_flow(3);                           -- có tên + UID đối tác
