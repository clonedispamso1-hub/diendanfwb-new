-- =====================================================================
-- 🎁 CLONE GIFT V4 — ĐƠN GIẢN HOÁ, KHÔNG NOTIFICATION (SUPABASE #1)
--
--   • Clone gửi bị TRỪ Xu, chủ bài viết được CỘNG Xu NGAY (không cần "Nhận").
--   • Ghi `post_gifts` (claimed = true) để bài viết hiển thị có người tặng.
--   • KHÔNG tạo notification, KHÔNG phụ thuộc Supabase #3.
--   • ATOMIC + IDEMPOTENT: cùng `p_idem` ⇒ không trừ/cộng lần 2.
--   • KHÔNG đụng luồng tặng quà của user thường (send_post_gift_v2 /
...
DROP FUNCTION IF EXISTS public.admin_clone_gift_post_v4(uuid, uuid, uuid, text, bigint, text);
CREATE FUNCTION public.admin_clone_gift_post_v4(
  p_account     uuid,
  p_post_id     uuid,
  p_receiver_id uuid,
  p_gift_key    text,
...
BEGIN
  ...
  -- (C) Ghi quà — đã tính là nhận xong (không cần bấm "Nhận").
  ...
  VALUES (p_account, p_receiver_id, p_amount, 'Tặng quà ' || v_key,
          'gift_v1', p_post_id, 'completed', now());
...
-- Dọn rác: bản v3 (2 pha + notification) không còn dùng.
