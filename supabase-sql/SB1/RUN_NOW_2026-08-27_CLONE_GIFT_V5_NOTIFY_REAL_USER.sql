-- =====================================================================
-- 🎁 CLONE GIFT V5 — CHẠY TRÊN SUPABASE #1 (core/auth/ví)
--
-- LUỒNG CHUẨN:
--   • Clone → USER THẬT (profiles.account_source <> 'internal'):
--       - TRỪ Xu clone ngay.
--       - GHI post_gifts với claimed = false (Xu TREO, chưa cộng cho ai).
--       - gem_transactions status = 'pending'.
--       - Trả về needs_notification = true ⇒ app ghi notification sang SB3
--         (RPC notify_post_gift_v5). Người nhận bấm "Nhận" ⇒ claim_post_gift_v2
--         mới CỘNG Xu + chạy hiệu ứng quà/Xu bay vào ví.
--   • Clone → CLONE (account_source = 'internal'):
--       - Trừ Xu clone gửi, CỘNG THẲNG Xu cho clone nhận, claimed = true,
--         gem_transactions 'completed', KHÔNG notification.
--
--   • ATOMIC + IDEMPOTENT theo p_idem.
--   • KHÔNG đụng luồng user thật tặng nhau (send_post_gift_v2 /
--     claim_post_gift_v2 giữ nguyên).
...
DROP FUNCTION IF EXISTS public.admin_clone_gift_post_v5(uuid, uuid, uuid, text, bigint, text);
CREATE FUNCTION public.admin_clone_gift_post_v5(
  p_account     uuid,
  p_post_id     uuid,
  p_receiver_id uuid,
  p_gift_key    text,
...
BEGIN
  ...
  VALUES (p_account, p_receiver_id, p_amount, 'Tặng quà ' || v_key,
          'gift_v1', p_post_id,
          CASE WHEN v_recv_clone THEN 'completed' ELSE 'pending' END, now());
...
REVOKE ALL ON FUNCTION public.admin_clone_gift_post_v5(uuid, uuid, uuid, text, bigint, text) FROM public;
GRANT EXECUTE ON FUNCTION public.admin_clone_gift_post_v5(uuid, uuid, uuid, text, bigint, text) TO authenticated;
