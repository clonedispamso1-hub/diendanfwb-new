-- =====================================================================
-- SB3_PURGE_MEMBER.sql  ·  Supabase #3 (logs / posts / comments / chat / stats)
-- ---------------------------------------------------------------------
-- CHỈ chạy file này trên Supabase #3. Không chạy trên #1 / #2.
--
-- Nguyên tắc an toàn (giống hệt SB1_PURGE_MEMBER.sql):
--   • CHỈ xoá dữ liệu có quan hệ trực tiếp với user: cột có FOREIGN KEY
--     tới public.profiles(id) (bảng mirror của #3), hoặc owner-column
...
--   • #3 KHÔNG có auth.users → tuyệt đối không chạm schema auth ở đây.
--   • Xoá tất cả: cần đúng cả hai — 'XOAHETDI' và '792006'.
--     Xoá 1 member thì KHÔNG cần hai mã đó.
--
-- Ghi chú xác thực: app đăng nhập ở #1; #3 được gọi bằng publishable key
-- (anon bridge). Vì vậy auth.uid() ở đây có thể NULL. Quy tắc:
--   - auth.uid() có giá trị  → bắt buộc phải là Admin của #3.
...
RETURNS text[] LANGUAGE sql IMMUTABLE AS $$
  SELECT ARRAY[
    ...
    'uploader_id','post_owner_id','commenter_id'
  ]::text[]
$$;
...
RETURNS TABLE(tbl text, col text) LANGUAGE sql STABLE AS $$
  -- (a) FK tới profiles(id) bảng profiles mirror của #3
...
-- ---------------------------------------------------------------------
-- 4) Xoá sạch dữ liệu #3 của 1 member
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  -- 4.2 profile mirror (nếu #3 có) — sau cùng
  IF to_regclass('public.profiles') IS NOT NULL THEN
  ...
  RETURN jsonb_build_object('ok', true, 'db', 'SB3', 'user_id', p_user_id, 'rows_deleted', deleted);
END; $$;
...
BEGIN
  ...
  -- (b) profile mirror còn sót trên #3 (không phải Admin)
  IF to_regclass('public.profiles') IS NOT NULL THEN
