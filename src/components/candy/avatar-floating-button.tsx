...
export function AvatarFloatingButton() {
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (Math.abs(targetX - pos.x) > 3 || Math.abs(targetY - pos.y) > 3)
  ...
  const popupStyle: React.CSSProperties =
    typeof window === "undefined"
      ...
      : {
          position: "fixed",
          [popupRight ? "right" : "left"]: popupRight
            ? Math.max(8, window.innerWidth - pos.x - SIZE)
            : Math.max(8, pos.x),
          [popupBelow ? "top" : "bottom"]: popupBelow
            ? pos.y + SIZE + 10
            : Math.max(8, window.innerHeight - pos.y + 10),
          zIndex: 9999,
          width: 196,
        };
...
function LinkChoiceModal({
  ...
  const openLink = (url?: string, fallback?: string) => {
    if (!url) {
      alert(`Link "${fallback}" chưa được admin cấu hình.`);
      return;
    }
        ...
        {open && (
          ...
          >
            ...
            >
              ...
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, paddingRight: 32 }}>
                {title}
              </h3>
              <p
                style={{
