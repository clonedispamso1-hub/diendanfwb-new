...
function normalize(raw: unknown): FeaturePopupConfig[] {
  ...
  for (const r of list) {
    ...
    byKey.set(item.key, { ...base, ...item, key: item.key });
  }
...
export async function loadFeaturePopups(): Promise<FeaturePopupConfig[]> {
  try {
    ...
    return normalize((data as any)?.items);
  } catch {
    return DEFAULT_POPUPS.map((p) => ({ ...p }));
  }
}

/** Lưu toàn bộ cấu hình popup (chỉ admin). */
export async function saveFeaturePopups(items: FeaturePopupConfig[]) {
