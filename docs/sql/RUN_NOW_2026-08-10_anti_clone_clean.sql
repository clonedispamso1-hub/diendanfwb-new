-- =====================================================================
-- ANTI CLONE / ANTI SPAM — CLEAN BUILD (2026-08-10)
-- 100% chạy được trên Supabase SQL Editor. Idempotent, additive.
-- KHÔNG chứa bất kỳ phần nào của CALL SYSTEM
-- (call_voice_url / call_video_url / call_media / call_sessions / call_logs).
-- KHÔNG đổi Supabase URL / API Key / Project. Không xoá dữ liệu hiện có.
-- Mọi ADD COLUMN đều nằm trong một câu ALTER TABLE hoàn chỉnh.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1) device_accounts — nguồn tín hiệu thiết bị (device_id, cookie_id, ip)
-- ---------------------------------------------------------------------
...
ALTER TABLE public.device_accounts ADD COLUMN IF NOT EXISTS device_id   text;
...
-- device_id mặc định = fingerprint (nếu chưa có)
...
-- ---------------------------------------------------------------------
-- 3) Nhật ký hoạt động thành viên
-- ---------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS member_activity_user_idx
...
-- ---------------------------------------------------------------------
-- 5) Ghi nhận tín hiệu thiết bị (gọi sau khi đăng nhập / mở app)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF prev.id IS NOT NULL AND prev.fingerprint IS DISTINCT FROM p_fingerprint THEN
    ...
    VALUES (uid, 'device_change', COALESCE(prev.fingerprint, '—') || ' -> ' || p_fingerprint, p_ip, p_fingerprint);
  ...
  UPDATE public.device_accounts SET
    ...
    cookie_id   = COALESCE(p_cookie_id, cookie_id),
    device_id   = COALESCE(p_device_id, device_id, p_fingerprint),
    last_seen   = now()
  WHERE user_id = uid AND fingerprint = p_fingerprint;

  IF NOT FOUND THEN
    INSERT INTO public.device_accounts(
      user_id, fingerprint, device_id, ip, user_agent, device_type, os,
      browser, country, isp, cookie_id, last_seen)
    VALUES (uid, p_fingerprint, COALESCE(p_device_id, p_fingerprint), p_ip, p_user_agent,
            p_device_type, p_os, p_browser, p_country, p_isp, p_cookie_id, now());
  END IF;
END $$;
GRANT EXECUTE ON FUNCTION public.register_device_signal(text,text,text,text,text,text,text,text,text,text) TO authenticated;

-- ---------------------------------------------------------------------
-- 6) Kiểm tra chặn trước khi đăng nhập / đăng ký
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF p_fingerprint IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.blocked_devices WHERE fingerprint = p_fingerprint) THEN
...
-- ---------------------------------------------------------------------
-- 7) VIEW: thiết bị mới nhất + nhóm IP / nhóm Device / cụm danh tính
-- ---------------------------------------------------------------------
CREATE OR REPLACE VIEW public.member_device_latest AS
  SELECT DISTINCT ON (d.user_id)
    d.user_id, d.fingerprint, COALESCE(d.device_id, d.fingerprint) AS device_id,
    d.ip, d.user_agent, d.device_type, d.os, d.browser, d.country, d.isp, d.cookie_id,
...
-- Nhóm theo IP
CREATE OR REPLACE VIEW public.ip_groups AS
  SELECT d.ip,
         COUNT(DISTINCT d.user_id)::int                            AS account_count,
         COUNT(DISTINCT COALESCE(d.device_id, d.fingerprint))::int AS device_count,
         MIN(d.created_at)                                         AS first_seen,
         MAX(COALESCE(d.last_seen, d.created_at))                  AS last_seen,
         COUNT(b.ip) > 0                                           AS is_blocked
    FROM public.device_accounts d
    LEFT JOIN public.blocked_ips b ON b.ip = d.ip
   WHERE d.ip IS NOT NULL
   GROUP BY d.ip;

-- Nhóm theo Device
CREATE OR REPLACE VIEW public.device_groups AS
  SELECT dd.device_key                                       AS device_id,
         COUNT(DISTINCT dd.user_id)::int                     AS account_count,
         COUNT(DISTINCT dd.ip)::int                          AS ip_count,
         MIN(dd.created_at)                                  AS first_seen,
         MAX(COALESCE(dd.last_seen, dd.created_at))          AS last_seen,
         BOOL_OR(b.fingerprint IS NOT NULL)                  AS is_blocked
    FROM (
      SELECT COALESCE(d.device_id, d.fingerprint) AS device_key,
             d.user_id, d.ip, d.created_at, d.last_seen
        FROM public.device_accounts d
       WHERE COALESCE(d.device_id, d.fingerprint) IS NOT NULL
    ) dd
    LEFT JOIN public.blocked_devices b ON b.fingerprint = dd.device_key
   GROUP BY dd.device_key;

-- Cụm danh tính (device / ip / cookie) có từ 2 tài khoản trở lên
CREATE OR REPLACE VIEW public.member_identity_clusters AS
  WITH k AS (
    SELECT 'device'::text AS cluster_type,
           COALESCE(d.device_id, d.fingerprint) AS cluster_key,
           d.user_id, d.ip, COALESCE(d.last_seen, d.created_at) AS seen
      FROM public.device_accounts d
     WHERE COALESCE(d.device_id, d.fingerprint) IS NOT NULL
    UNION ALL
    SELECT 'ip', d.ip, d.user_id, d.ip, COALESCE(d.last_seen, d.created_at)
      FROM public.device_accounts d
     WHERE d.ip IS NOT NULL
    UNION ALL
    SELECT 'cookie', d.cookie_id, d.user_id, d.ip, COALESCE(d.last_seen, d.created_at)
      FROM public.device_accounts d
     WHERE d.cookie_id IS NOT NULL
  )
  SELECT k.cluster_type,
         k.cluster_key,
         COUNT(DISTINCT k.user_id)::int AS account_count,
         COUNT(DISTINCT k.ip)::int      AS ip_count,
         MAX(k.seen)                    AS last_seen,
         ARRAY_AGG(DISTINCT k.user_id)  AS user_ids,
         LEAST(100, COUNT(DISTINCT k.user_id)::int
                    * CASE WHEN k.cluster_type = 'ip' THEN 8 ELSE 12 END)::int AS risk_score
    FROM k
   GROUP BY k.cluster_type, k.cluster_key
  HAVING COUNT(DISTINCT k.user_id) >= 2;

-- ---------------------------------------------------------------------
-- 8) RPC chính: admin_member_intel — danh sách thành viên + risk_score
-- p_flags: dup_ip | dup_device | dup_cookie | risk60 | risk80
--          | acc3 | acc5 | acc10 | clone | spam
-- ---------------------------------------------------------------------
...
RETURNS TABLE (
  ...
  fingerprint text, device_id text, ip text, device_type text, os text, browser text,
...
BEGIN
  ...
  ipc AS (SELECT d.ip, COUNT(DISTINCT d.user_id)::int c
            FROM public.device_accounts d WHERE d.ip IS NOT NULL GROUP BY d.ip),
  fpc AS (SELECT COALESCE(d.device_id, d.fingerprint) k, COUNT(DISTINCT d.user_id)::int c
            FROM public.device_accounts d
           WHERE COALESCE(d.device_id, d.fingerprint) IS NOT NULL GROUP BY 1),
  ckc AS (SELECT d.cookie_id, COUNT(DISTINCT d.user_id)::int c
            FROM public.device_accounts d WHERE d.cookie_id IS NOT NULL GROUP BY d.cookie_id),
  ipch AS (SELECT d.user_id, COUNT(DISTINCT d.ip)::int c
             FROM public.device_accounts d WHERE d.ip IS NOT NULL GROUP BY d.user_id),
  ...
  calc AS (
    SELECT b.*,
      dv.fingerprint, dv.device_id, dv.ip, dv.device_type, dv.os, dv.browser,
      dv.country, dv.isp, dv.cookie_id, dv.device_seen_at,
    ...
    LEFT JOIN fpc    ON fpc.k = dv.device_id
  ...
  scored AS (
    SELECT c.*,
      ...
      ARRAY_REMOVE(ARRAY[
        ...
        CASE WHEN c.name_twin_count   > 1 THEN 'Tên trùng tài khoản khác' END,
        CASE WHEN c.avatar_twin_count > 1 THEN 'Avatar trùng tài khoản khác' END
  ...
  SELECT f.id, f.username, f.full_name, f.avatar, f.phone, f.is_admin, f.is_banned,
         f.created_at, f.last_seen, f.fingerprint, f.device_id, f.ip, f.device_type,
         f.os, f.browser, f.country, f.isp, f.cookie_id, f.device_seen_at,
...
-- ---------------------------------------------------------------------
-- 9) Chi tiết cụm theo IP / Device
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.admin_cluster_detail(p_scope text, p_key text)
RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  info jsonb; accounts jsonb; ips jsonb; devices jsonb; blocked boolean;
BEGIN
  IF NOT public.mi_is_admin() THEN
    RAISE EXCEPTION 'forbidden';
  END IF;
  ...
  SELECT jsonb_build_object(
      ...
      'os', max(d.os), 'device_type', max(d.device_type), 'user_agent', max(d.user_agent),
      'first_seen', min(d.created_at), 'last_seen', max(COALESCE(d.last_seen, d.created_at)))
    INTO info
    FROM public.device_accounts d
   WHERE (p_scope = 'ip' AND d.ip = p_key)
      OR (p_scope = 'device' AND COALESCE(d.device_id, d.fingerprint) = p_key);

  SELECT COALESCE(jsonb_agg(s.x), '[]'::jsonb) INTO accounts
    FROM (
  ...
  SELECT COALESCE(jsonb_agg(DISTINCT d.ip), '[]'::jsonb) INTO ips
    FROM public.device_accounts d
   WHERE d.ip IS NOT NULL
     AND ((p_scope = 'ip' AND d.ip = p_key)
          OR (p_scope = 'device' AND COALESCE(d.device_id, d.fingerprint) = p_key));

  SELECT COALESCE(jsonb_agg(DISTINCT COALESCE(d.device_id, d.fingerprint)), '[]'::jsonb) INTO devices
    FROM public.device_accounts d
   WHERE COALESCE(d.device_id, d.fingerprint) IS NOT NULL
     AND ((p_scope = 'ip' AND d.ip = p_key)
          OR (p_scope = 'device' AND COALESCE(d.device_id, d.fingerprint) = p_key));
  ...
  RETURN jsonb_build_object('scope', p_scope, 'key', p_key,
    'info', COALESCE(info, '{}'::jsonb), 'accounts', accounts,
    'ips', ips, 'devices', devices, 'blocked', blocked,
...
-- Danh sách cụm danh tính cho bảng admin
...
BEGIN
  ...
  RETURN QUERY
  WITH c AS (
    SELECT mic.cluster_key, mic.account_count, mic.ip_count, mic.last_seen,
           mic.user_ids, mic.risk_score
      FROM public.member_identity_clusters mic
     WHERE mic.cluster_type = COALESCE(p_scope, 'device')
  )
  SELECT c.cluster_key, c.account_count, c.ip_count,
         (SELECT COUNT(*)::int FROM public.profiles p
           WHERE p.id = ANY(c.user_ids) AND COALESCE(p.is_banned, false)) AS banned_count,
         c.last_seen,
         (SELECT ARRAY_AGG(p.username) FROM public.profiles p WHERE p.id = ANY(c.user_ids)) AS usernames,
         c.risk_score,
         COUNT(*) OVER () AS total_count
    FROM c
   WHERE c.account_count >= COALESCE(p_min_accounts, 2)
...
-- ---------------------------------------------------------------------
-- 10) Khoá thủ công: device / ip / member
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.admin_lock_device(
  p_device_id text, p_reason text DEFAULT NULL, p_lock boolean DEFAULT true)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  IF p_lock THEN
    ...
    VALUES (p_device_id, COALESCE(p_reason, 'admin_lock_device'), auth.uid())
    ON CONFLICT (fingerprint) DO NOTHING;
  ELSE
    DELETE FROM public.blocked_devices WHERE fingerprint = p_device_id;
  END IF;

  INSERT INTO public.member_activity_log(user_id, action, detail, fingerprint)
  SELECT DISTINCT d.user_id,
         CASE WHEN p_lock THEN 'device_locked' ELSE 'device_unlocked' END,
         COALESCE(p_reason, 'Admin thao tác khoá thiết bị'),
         p_device_id
    FROM public.device_accounts d
   WHERE d.user_id IS NOT NULL
     AND COALESCE(d.device_id, d.fingerprint) = p_device_id;
  GET DIAGNOSTICS affected = ROW_COUNT;

  RETURN jsonb_build_object('ok', true, 'device_id', p_device_id,
                            'locked', p_lock, 'users_logged', affected);
END $$;
GRANT EXECUTE ON FUNCTION public.admin_lock_device(text,text,boolean) TO authenticated;

CREATE OR REPLACE FUNCTION public.admin_lock_ip(
  p_ip text, p_reason text DEFAULT NULL, p_lock boolean DEFAULT true)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE affected int := 0;
BEGIN
  ...
  IF p_lock THEN
    ...
    VALUES (p_ip, COALESCE(p_reason, 'admin_lock_ip'), auth.uid())
    ON CONFLICT (ip) DO NOTHING;
  ELSE
    DELETE FROM public.blocked_ips WHERE ip = p_ip;
  END IF;

  INSERT INTO public.member_activity_log(user_id, action, detail, ip)
  SELECT DISTINCT d.user_id,
         CASE WHEN p_lock THEN 'ip_locked' ELSE 'ip_unlocked' END,
         COALESCE(p_reason, 'Admin thao tác khoá IP'),
         p_ip
    FROM public.device_accounts d
   WHERE d.user_id IS NOT NULL AND d.ip = p_ip;
  GET DIAGNOSTICS affected = ROW_COUNT;

  RETURN jsonb_build_object('ok', true, 'ip', p_ip,
                            'locked', p_lock, 'users_logged', affected);
END $$;
GRANT EXECUTE ON FUNCTION public.admin_lock_ip(text,text,boolean) TO authenticated;

-- Khoá thành viên 3 mức: 1 = account, 2 = + device, 3 = + ip
CREATE OR REPLACE FUNCTION public.admin_lock_member(
  p_user uuid,
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_device text; v_ip text; v_until timestamptz;
BEGIN
  IF NOT public.mi_is_admin() THEN
    RAISE EXCEPTION 'forbidden';
  END IF;

  SELECT d.device_id, d.ip INTO v_device, v_ip
    FROM public.member_device_latest d WHERE d.user_id = p_user;
  ...
  IF NOT p_lock THEN
    ...
    DELETE FROM public.blocked_devices WHERE fingerprint IN (
      SELECT DISTINCT COALESCE(d.device_id, d.fingerprint) FROM public.device_accounts d
       WHERE d.user_id = p_user AND COALESCE(d.device_id, d.fingerprint) IS NOT NULL);
    DELETE FROM public.blocked_ips WHERE ip IN (
      SELECT DISTINCT d.ip FROM public.device_accounts d
       WHERE d.user_id = p_user AND d.ip IS NOT NULL);

    INSERT INTO public.member_activity_log(user_id, action, detail, ip, fingerprint)
    VALUES (p_user, 'unban', COALESCE(p_reason, 'Mở khoá toàn bộ'), v_ip, v_device);

    RETURN jsonb_build_object('ok', true, 'locked', false, 'user_id', p_user);
  END IF;

  v_until := CASE WHEN COALESCE(p_days, 0) > 0 THEN now() + (p_days || ' days')::interval END;

  UPDATE public.profiles SET is_banned = true, banned_until = v_until WHERE id = p_user;

  IF COALESCE(p_level, 1) >= 2 AND v_device IS NOT NULL THEN
    INSERT INTO public.blocked_devices(fingerprint, reason, created_by)
    VALUES (v_device, COALESCE(p_reason, 'admin_lock_member_l' || p_level), auth.uid())
    ON CONFLICT (fingerprint) DO NOTHING;
  END IF;

  IF COALESCE(p_level, 1) >= 3 AND v_ip IS NOT NULL THEN
    INSERT INTO public.blocked_ips(ip, reason, created_by)
    VALUES (v_ip, COALESCE(p_reason, 'admin_lock_member_l' || p_level), auth.uid())
    ON CONFLICT (ip) DO NOTHING;
  END IF;

  INSERT INTO public.member_activity_log(user_id, action, detail, ip, fingerprint)
  VALUES (p_user, 'ban',
          'Khoá mức ' || COALESCE(p_level, 1) || COALESCE(' — ' || p_reason, ''),
          v_ip, v_device);

  RETURN jsonb_build_object('ok', true, 'locked', true, 'user_id', p_user,
    'level', COALESCE(p_level, 1), 'device_id', v_device, 'ip', v_ip,
    'banned_until', v_until);
END $$;
GRANT EXECUTE ON FUNCTION public.admin_lock_member(uuid,int,text,int,boolean) TO authenticated;
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT public.admin_lock_member(p_user, p_level, p_reason, p_days, true);
$$;
...
RETURNS jsonb
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT public.admin_lock_member(p_user, 1, 'Mở khoá toàn bộ', 0, false);
$$;
GRANT EXECUTE ON FUNCTION public.admin_unban_member_full(uuid) TO authenticated;

-- ---------------------------------------------------------------------
-- 11) Hành động hàng loạt trên cụm
-- p_action: ban_all | unban_all | logout_all | mark_spam | block | unblock
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.admin_cluster_action(
  p_scope text, p_key text, p_action text, p_reason text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  SELECT ARRAY_AGG(DISTINCT d.user_id) INTO uids
    FROM public.device_accounts d
   WHERE d.user_id IS NOT NULL
     AND ((p_scope = 'ip' AND d.ip = p_key)
          OR (p_scope = 'device' AND COALESCE(d.device_id, d.fingerprint) = p_key));
  ...
  IF p_action = 'ban_all' THEN
    UPDATE public.profiles SET is_banned = true
     WHERE id = ANY(uids) AND NOT COALESCE(is_admin, false);
    GET DIAGNOSTICS n = ROW_COUNT;
    INSERT INTO public.member_activity_log(user_id, action, detail, ip, fingerprint)
      SELECT u, 'ban', COALESCE(p_reason, 'Khoá theo cụm ' || p_scope),
             CASE WHEN p_scope = 'ip' THEN p_key END,
             CASE WHEN p_scope = 'device' THEN p_key END
        FROM unnest(uids) u;
  ...
  ELSIF p_action = 'unban_all' THEN
    ...
    DELETE FROM public.blocked_devices WHERE fingerprint IN (
      SELECT DISTINCT COALESCE(d.device_id, d.fingerprint) FROM public.device_accounts d
       WHERE d.user_id = ANY(uids) AND COALESCE(d.device_id, d.fingerprint) IS NOT NULL);
    DELETE FROM public.blocked_ips WHERE ip IN (
      SELECT DISTINCT d.ip FROM public.device_accounts d
       WHERE d.user_id = ANY(uids) AND d.ip IS NOT NULL);
    INSERT INTO public.member_activity_log(user_id, action, detail)
      SELECT u, 'unban', 'Mở khoá theo cụm ' || p_scope FROM unnest(uids) u;
  ...
  ELSIF p_action = 'mark_spam' THEN
    INSERT INTO public.member_activity_log(user_id, action, detail)
      SELECT u, 'spam', COALESCE(p_reason, 'Admin đánh dấu Spam (cụm ' || p_scope || ')')
        FROM unnest(uids) u;
    n := COALESCE(array_length(uids, 1), 0);

  ELSIF p_action = 'block' THEN
    IF p_scope = 'ip' THEN
      PERFORM public.admin_lock_ip(p_key, p_reason, true);
    ELSE
      PERFORM public.admin_lock_device(p_key, p_reason, true);
    END IF;
    n := 1;

  ELSIF p_action = 'unblock' THEN
    IF p_scope = 'ip' THEN
      PERFORM public.admin_lock_ip(p_key, p_reason, false);
    ELSE
      PERFORM public.admin_lock_device(p_key, p_reason, false);
    END IF;
    n := 1;
...
-- Quyền đọc view cho admin (view chạy theo quyền người gọi + RLS bảng gốc)
GRANT SELECT ON public.member_device_latest      TO authenticated;
GRANT SELECT ON public.ip_groups                 TO authenticated;
GRANT SELECT ON public.device_groups             TO authenticated;
GRANT SELECT ON public.member_identity_clusters  TO authenticated;
